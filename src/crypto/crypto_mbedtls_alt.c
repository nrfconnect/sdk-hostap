...
/**
 * Based on WPA-Supplicant Internal implementation. The API invokes
 * PSA APIs instead of invoking MBEDTLS APIs
 */
static int hmac_prf256_psa(const u8 *key,
                           size_t key_len,
                           const char *label,
                           const u8 *data,
                           size_t data_len,
...
{
#ifdef CONFIG_WIFI_NM_WPA_SUPPLICANT_CRYPTO_MBEDTLS_PSA
    return hmac_prf256_psa(key, key_len, label, data, data_len, buf, buf_len * 8, MBEDTLS_MD_SHA256);
#else
